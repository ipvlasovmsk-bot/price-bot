...
if not BOT_TOKEN: raise ValueError("❌ Не установлен BOT_TOKEN")
if not ADMIN_IDS_STR: raise ValueError("❌ Не установлены ADMIN_IDS")
try:
...
class DB:
    ...
    def _load(self):
        ...
        return {"subscribers": {}, "price_lists": {}, "campaigns": {}, "campaign_stats": {}, "next_price_id": 1, "next_campaign_id": 1}
    def _save(self):
    ...
    async def add_sub(self, uid, u, fn, ln):
        ...
        self.data["subscribers"][uid_str] = {
            "username": u or "", "first_name": fn or "", "last_name": ln or "",
            "subscribed_at": now if is_new else existing.get("subscribed_at", now),
            "is_active": True, "unsubscribed_at": None
        }
        self._save()
        return "new" if is_new else ("reactivated" if is_reactivation else "existing")
    async def rem_sub(self, uid):
        ...
        if uid_str in self.data["subscribers"]:
            ...
            self._save()
    async def get_active(self): return [int(uid) for uid, d in self.data["subscribers"].items() if d.get("is_active", False)]
    async def add_price(self, fid, fn, ub):
        pid = self.data["next_price_id"]
        self.data["price_lists"][str(pid)] = {"id": pid, "file_id": fid, "file_name": fn, "uploaded_at": datetime.now().isoformat(), "uploaded_by": ub}
        self.data["next_price_id"] += 1
        self._save()
        return pid
    async def get_prices(self): return sorted([v for v in self.data["price_lists"].values()], key=lambda x: x["uploaded_at"], reverse=True)
    async def create_campaign(self, plid, sb):
        cid = self.data["next_campaign_id"]
        self.data["campaigns"][str(cid)] = {"id": cid, "price_list_id": plid, "scheduled_at": datetime.now().isoformat(), "sent_at": None, "sent_by": sb, "total_sent": 0, "total_failed": 0, "total_opened": 0}
    ...
    async def track_open(self, cid, uid, method="button"):
        ...
        if cid_str not in self.data["campaign_stats"]: self.data["campaign_stats"][cid_str] = {}
        if uid_str not in self.data["campaign_stats"][cid_str]: self.data["campaign_stats"][cid_str][uid_str] = {"sent_at": datetime.now().isoformat(), "delivered": False, "error_message": None, "opened_at": None, "opened_method": None}
    ...
    async def get_stats(self, cid):
        ...
        return {"total": total, "delivered": delivered, "opened": opened, "failed": failed, "open_rate": round(opened / delivered * 100, 2) if delivered > 0 else 0}
    async def get_overall(self):
        ...
        return {"subscribers": subs, "total_campaigns": len(camps), "total_sent": total_sent, "total_opened": total_opened, "avg_open_rate": avg_rate, "new_subs_week": new_subs, "unsub_week": unsub}
...
async def start(m: Message):
    ...
    if status in ("new", "reactivated"):
        asyncio.create_task(notify_admins_about_new_subscriber(u, is_reactivation=(status == "reactivated")))
...
async def stop(m: Message):
    ...
    await m.answer("❌ Вы отписались от рассылки прайс-листов.\nЧтобы подписаться снова, напишите /start")
...
async def admin(m: Message):
    if not is_admin(m.from_user.id): await m.answer("🚫 Доступ запрещён"); return
    await m.answer("👑 <b>Панель администратора</b>\n\nКоманды:\n/upload — загрузить прайс-лист\n/send — отправить рассылку\n/stats — статистика\n/campaigns — история рассылок", parse_mode="HTML")
...
async def send_start(m: Message, state: FSMContext):
    ...
    if not prices: await m.answer("📭 Нет загруженных прайс-листов. Сначала /upload"); return
...
async def send_choice(m: Message, state: FSMContext):
    if not is_admin(m.from_user.id): return
    try: pid = int(m.text)
    except: await m.answer("❌ Неверный номер. Попробуйте снова."); return
    data = await state.get_data()
    prices = data.get("prices", {})
    if pid not in prices: await m.answer("❌ Прайс-лист не найден."); return
...
async def send_final(m: Message, state: FSMContext):
    ...
    if not price: await m.answer("❌ Ошибка выбора прайс-листа"); return
...
async def send_task(cid, fid, caption, admin_msg):
    ...
    if not subs: await admin_msg.answer("📭 Нет активных подписчиков"); return
    ...
    await admin_msg.answer(f"✅ Рассылка завершена!\n🆔 ID: {cid}\n📤 Отправлено: {sent}\n❌ Ошибок: {failed}\n📊 /campaigns для деталей")

async def send_user(uid, cid, fid, caption):
    try:
        if str(cid) not in db.data["campaign_stats"]: db.data["campaign_stats"][str(cid)] = {}
        db.data["campaign_stats"][str(cid)][str(uid)] = {"sent_at": datetime.now().isoformat(), "delivered": False, "error_message": None, "opened_at": None, "opened_method": None}
        db._save()
...
async def stats(m: Message):
    ...
    await m.answer(f"📊 <b>Статистика бота</b>\n\n👥 Всего: {s['subscribers']['total']}\n✅ Активных: {s['subscribers']['active']}\n\n📬 Рассылок: {s['total_campaigns']}\n📤 Отправлено: {s['total_sent']}\n👁️ Открыто: {s['total_opened']}\n📈 Открытий: {s['avg_open_rate']}%\n\n🆕 Новых за неделю: {s['new_subs_week']}\n❌ Отписалось: {s['unsub_week']}", parse_mode="HTML")

@router.message(Command("campaigns"))
async def campaigns(m: Message):
    if not is_admin(m.from_user.id): return
    camps = sorted([{"id": int(k), **v} for k, v in db.data["campaigns"].items()], key=lambda x: x.get("sent_at") or x.get("scheduled_at"), reverse=True)[:5]
    if not camps: await m.answer("📭 Нет рассылок"); return
    text = "📋 <b>История рассылок</b>\n\n"
    for c in camps:
        sent_at = c.get("sent_at") or c.get("scheduled_at")
        try: sent_at = datetime.fromisoformat(sent_at).strftime("%d.%m %H:%M")
        except: sent_at = "—"
        st = await db.get_stats(c["id"])
        text += f"🆔 {c['id']} | {c.get('file_name', '—')}\n🕐 {sent_at}\n📤 {st['delivered']} 👁️ {st['opened']} ({st['open_rate']}%)\n❌ {st['failed']}\n────────────\n"
...
=======
    asyncio.run(main())
>>>>>>> 8b76a277a2b35b1b893e0498b96f4ee84e92a9af
